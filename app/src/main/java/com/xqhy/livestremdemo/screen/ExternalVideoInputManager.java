...
import com.xqhy.livestremdemo.screen.share.ShowScreenShareInput;
...
import static com.xqhy.livestremdemo.screen.ScreenConstants.ENGINE;
import static com.xqhy.livestremdemo.screen.ScreenConstants.TEXTUREVIEW;
...
public class ExternalVideoInputManager implements IVideoSource {
    ...
    boolean setExternalVideoInput(int type, Intent intent) {
        ...
        switch (type) {
            case TYPE_SCREEN_SHARE:
                ...
                if (TEXTUREVIEW != null) {
                    input = new ShowScreenShareInput(context, width, height, dpi, fps, intent);
                    // add TextureView时触发
                    TEXTUREVIEW.setSurfaceTextureListener((ShowScreenShareInput)input);
                }else{
    ...
    private class ExternalVideoInputThread extends Thread {
        ...
        private void prepare() {
            ...
            // 设置自定义视频源
            ScreenConstants.ENGINE.setVideoSource(ExternalVideoInputManager.this);
        }
